...
class ListService extends Service{
    ...
    async saveOrUpdateList(data){
        const dataid = await this.app.mysql.get('base_list',{id:data.id});
        if (dataid) {
            var res = await this.app.mysql.update('base_list',data);
        }else{
            var res = await this.app.mysql.insert('base_list',data);
        }
        return res;
    }
    // 分页查询订单数据
